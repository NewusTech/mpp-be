...
module.exports = {
    ...
    try {
      ...
      let instansiCreateObj = {
        ...
        active_offline: req.body.active_offline
          ? Number(req.body.active_offline)
          : 0,
        active_online: req.body.active_online
          ? Number(req.body.active_online)
          : 0,
      ...
      res
        .status(201)
        .json(response(201, "success create instansi", instansiCreate));
    } catch (err) {
  ...
  getinstansiBySlug: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(
          response(200, "success get instansi by slug", formattedInstansiGets)
        );
    } catch (err) {
      res.status(500).json(response(500, "internal server error", err));
      console.log(err);
  ...
  updateinstansi: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json(response(200, "success update instansi", instansiAfterUpdate));
    } catch (err) {
      res.status(500).json(response(500, "internal server error", err));
      console.log(err);
    }
    ...
    try {
      ...
      const report_perinstansi = instansi.map((inst) => {
        ...
        const totalStatus3 = allLayanan.reduce((sum, lay) => {
          return (
            sum + lay.Layananformnums.filter((fn) => fn.status === 3).length
          );
        }, 0);

        const totalStatus4 = allLayanan.reduce((sum, lay) => {
          return (
            sum + lay.Layananformnums.filter((fn) => fn.status === 4).length
          );
        }, 0);
  ...
  pdfreportkinerja: async (req, res) => {
    try {
      ...
      const reportTableRows = report_perlayanan
        .map(
          (layanan) => `
                <tr>
                    <td>${layanan.name}</td>
      ...
      const reportTableRows2 = report_perinstansi
        .map(
          (instansi) => `
                <tr>
  ...
  pdfreportdokumen: async (req, res) => {
    try {
      ...
      const templatePath = path.resolve(
        __dirname,
        "../views/laporandokumenterbit.html"
      );
      let htmlContent = fs.readFileSync(templatePath, "utf8");
  ...
  reportpermasalahan: async (req, res) => {
    try {
      ...
      const pagination = generatePagination(
        ...
        page,
        limit,
        "/api/user/instansi/reportmasalah"
      );
  ...
  pdfreportpermasalahan: async (req, res) => {
    try {
      ...
      const instansiInfo = instansiGet?.name
        ? `<p>Instansi : ${instansiGet?.name}</p>`
        : "";
      const layananInfo = pengaduanGet?.name
        ? `<p>Layanan : ${pengaduanGet?.name}</p>`
        : "";
      let tanggalInfo = "";
      if (start_date || end_date) {
        const startDateFormatted = start_date
      ...
      const reportTableRows = pengaduanGets[0]
        ?.map((pengaduan) => {
          const createdAtDate = new Date(
            pengaduan.createdAt
          ).toLocaleDateString("id-ID", {
          ...
          return `
                     <tr>
                         ...
                         <td>${
                           pengaduan?.Admin?.name ? pengaduan?.Admin?.name : "-"
                         }</td>
  ...
  reportgeolayanan: async (req, res) => {
    try {
      ...
      const pagination = generatePagination(
        ...
        page,
        limit,
        "/api/user/instansi/reportgeolayanan"
      );
  ...
  pdfgeolayanan: async (req, res) => {
    try {
      ...
      const reportTableRows = formattedData
        ?.map((lastdata) => {
          const createdAtDate = new Date(lastdata.createdAt).toLocaleDateString(
            "id-ID",
  ...
  getScreenAntrian: async (req, res) => {
    try {
      ...
      const transformedData = layananGets?.map((layanan) => ({
        ...
        code: layanan?.code,
        antrian_now:
          layanan?.Antrians?.length > 0 ? layanan?.Antrians[0]?.code : "-",
      }));
